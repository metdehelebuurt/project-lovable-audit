...
const SolarPotentieCheck = ({ adres, postcode, plaats, compact }: Props) => {
  ...
  const check = async () => {
    if (!fullAddress || !MAPS_API_KEY) return;
    setLoading(true);
    setError(null);

    try {
      // Geocode first
      const geoUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(fullAddress)}&key=${MAPS_API_KEY}`;
  ...
  if (!fullAddress || !MAPS_API_KEY) return null;
