...
interface Props {
  ...
  doc: { id: string; documentnummer: string; partner_id: string };
  defaultTo: string;
...
export default function FactuurEmailDialog({ open, onOpenChange, doc, defaultTo, pdfElementSelector = ".pdf-print-root", onSent }: Props) {
  ...
  const [subject, setSubject] = useState(`Factuur ${doc.documentnummer}`);
  const [body, setBody] = useState(`Beste klant,\n\nHierbij ontvangt u onze factuur ${doc.documentnummer}.\n\nMet vriendelijke groet`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader><DialogTitle>Factuur per e-mail versturen</DialogTitle></DialogHeader>
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Paperclip className="h-3.5 w-3.5" /> PDF van de factuur wordt automatisch bijgevoegd
          </div>
