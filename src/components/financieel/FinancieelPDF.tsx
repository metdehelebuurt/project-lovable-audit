...
const typeSubtitles: Record<string, string> = {
  ...
  creditnota: "Creditnota",
  inkoopfactuur: "Inkoopfactuur",
  inkooporder: "Bestelling",
  pakbon: "Pakbon / Afleverbon",
};
...
export function FinancieelPDF({ doc, klant, leverancier, partner, installatie }: Props) {
  ...
  const netttoTotaal = regels.reduce((s, r) => s + regelSubtotaal(r), 0);
  const kortingTotaal = brutoTotaal - netttoTotaal;
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12mm", gap: "10mm" }}>
        ...
        {relatie && (
          <div style={{ flex: 1, padding: "12px 16px", backgroundColor: "#f9fafb", borderRadius: "6px", border: "1px solid #e5e7eb" }}>
            ...
            {"btw_nummer" in relatie && relatie.btw_nummer && <div style={{ color: "#6b7280" }}>BTW: {relatie.btw_nummer}</div>}
          </div>
        )}

        {/* Meta info */}
        <div style={{ width: "55mm", fontSize: "8pt" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "8mm" }}>
        <thead>
          <tr style={{ borderBottom: `2px solid ${primaryColor}` }}>
            ...
            {!isPakbon(doc.type) && (
              <>
                <th style={{ textAlign: "right", padding: "6px 4px", fontWeight: 600, fontSize: "8pt", width: "70px" }}>Prijs</th>
        ...
        <tbody>
          {regels.map((r, i) => (
            <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
              <td style={{ padding: "6px 4px" }}>{r.omschrijving}</td>
      ...
      {showPricing(doc.type) && (
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "12mm" }}>
          <div style={{ width: "65mm" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", fontSize: "8.5pt" }}>
              <span style={{ color: "#6b7280" }}>BTW</span>
