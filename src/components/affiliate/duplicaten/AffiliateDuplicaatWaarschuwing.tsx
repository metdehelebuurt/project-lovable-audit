...
import { useAffiliateDuplicatenVoorLead } from "./useAffiliateDuplicaten";
...
export function AffiliateDuplicaatWaarschuwing({ leadId }: Props) {
  const { data = [], isLoading } = useAffiliateDuplicatenVoorLead(leadId);
      ...
      <div className="rounded-xl border border-warning/40 bg-warning-light/60 p-4 space-y-3">
        ...
        <div className="space-y-2">
          {data.map((p) => {
            ...
            const titel = ander.bedrijfsnaam || ander.contactpersoon || ander.email || "Onbekend";
            return (
              <div key={`${p.lead_a_id}-${p.lead_b_id}`} className="rounded-lg bg-background border p-3 flex items-center gap-3 flex-wrap">
                <div className="flex-1 min-w-[200px]">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {[ander.email, ander.telefoon, ander.website].filter(Boolean).join(" · ") || "—"}
                  </p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">
                    Matcht op {p.match_redenen.map((r) => AFFILIATE_REDEN_LABELS[r] ?? r).join(", ")}
