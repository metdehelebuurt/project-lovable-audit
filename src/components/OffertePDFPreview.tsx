...
import { DEFAULT_SECTION_ORDER } from "@/components/offertes/templates/templateRegistry";
...
const sectionLabels: Record<string, string> = {
  voorblad: "Voorblad", inhoudsopgave: "Inhoudsopgave", producten: "Producten",
  prijstabel: "Offerte & Voorwaarden", energieadvies: "Besparing & Rendement",
  schouwrapport: "Schouwrapport", datasheets: "Technische specificaties",
};
...
export default function OffertePDFPreview({ templateConfigOverride, hideActionBar }: OffertePDFPreviewProps = {}) {
        ...
        if (p) {
          ...
          if (p.feature_flags_json && typeof p.feature_flags_json === "object") {
            ...
            if (flags.offerte_template) {
              setTemplateConfig((prev: any) => ({ ...flags.offerte_template, ...prev }));
            }
          }
    ...
    if (schouw?.gegevens) {
      ...
      if (wp > 0 && verbruik > 0) {
        ...
        if (capaciteit >= 1) energieadvies = {
          ...
          co2Reductie: Math.round(extraZelf * 0.4),
          maandBesparing: Math.round(besparing / 12),
          besparingLevensduur: besparing * CONFIG.levensduur_jaren,
          zelfvoorzieningsgraad: verbruik > 0 ? Math.round((jaarOpwekking * CONFIG.zelfconsumptie_met_batterij / verbruik) * 100) : 0,
        };
      }
    }
  ...
  const tc = {
    voorblad: templateConfig?.secties_voorblad ?? true,
    productpagina: templateConfig?.secties_producten ?? true,
    energieadvies: templateConfig?.secties_energieadvies ?? true,
    schouwrapport: templateConfig?.secties_schouwrapport ?? true,
  ...
  const PageNumber = ({ num }: { num: number }) => (
    <div style={{ position: "absolute", bottom: 12, right: 20, fontSize: 9, color: "#bbb", fontFamily: "'Rubik', sans-serif" }}>{num}</div>
  );
  ...
  const renderGroupedSchouw = () => {
    ...
    fields.forEach(f => {
      ...
      if (val === true) displayVal = "Ja";
      if (val === false) displayVal = "Nee";
  ...
  const renderSection = (sectionId: string) => {
    switch (sectionId) {
      ...
      case "inhoudsopgave": {
        ...
        const tocItems = sectionOrder
          .filter(s => {
            ...
            if (s === "voorblad" && !tc.voorblad) return false;
        ...
        return (
          <div key="inhoudsopgave" className="pdf-page" style={pageStyle}>
            <PageHeader />
            <div style={{ flex: 1 }}>
      ...
      case "prijstabel": {
        pageNum++;
        return (
          <div key="prijstabel" className="pdf-page" style={pageStyle}>
            <PageHeader />
            <div style={{ flex: 1 }}>
      ...
      case "energieadvies":
        if (!tc.energieadvies || !energieadvies) return null;
        pageNum++;
        return (
          <div key="energieadvies" className="pdf-page" style={pageStyle}>
            <PageHeader />
            <div style={{ flex: 1 }}>
