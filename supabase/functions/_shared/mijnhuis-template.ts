export function wrapInMijnhuisTemplate(htmlBody: string, senderName?: string | null): string {
  const safeName = (senderName ?? "").trim();
  const signature = `<p style="margin:24px 0 4px;color:#0f172a;font-weight:600;">Met vriendelijke groet,</p>
       <p style="margin:0;color:#0f172a;">${escapeHtml(safeName || "Team mijnhuis.nu")}</p>`;

  return `<!doctype html>
<html lang="nl">
  <head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /><title>mijnhuis.nu</title></head>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f4f4f7;padding:24px 12px;">
      <tr><td align="center">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(15,23,42,0.06);">
          ...
          <tr><td style="background-color:#f8fafc;padding:18px 28px;border-top:1px solid #e2e8f0;font-size:12px;color:#64748b;text-align:center;">
